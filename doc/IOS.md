...
```objective-c
// import header
#import "FlutterFlavorConfigPlugin.h"

// then read individual keys like:
NSString *apiUrl = [FlutterFlavorConfigPlugin envFor:@"API_URL"];

// or just fetch the whole config
NSDictionary *config = [FlutterFlavorConfigPlugin env];
```
...
3. under Build > Pre-actions you need to add 2 new run script actions with the following code:
   ...
   ```
   ${SRCROOT}/.symlinks/plugins/flutter_flavor_config/ios/Classes/BuildXCConfig.rb ${SRCROOT}/ ${SRCROOT}/Flutter/tmp.xcconfig
   ```
